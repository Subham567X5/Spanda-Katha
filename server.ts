...
const app = express();
app.use(express.json({ limit: '50mb' })); // support larger base64 uploads and edits

// Dynamic port so the Electron host can pick a free one and avoid conflicts.
...
app.post("/api/generate-image", async (req, res) => {
  try {
    ...
    // If base64Image is provided, clean and include it for editing
    if (base64Image) {
    ...
    // Extract the image part from candidate content parts
...
// API route for detecting book details from an uploaded or designed cover image
app.post("/api/detect-book-details", async (req, res) => {
  try {
    ...
    // Prepare fallback data just in case
    ...
    // Lazy load AI client. If it fails due to no key, return fallback gracefully so development stays live.
    let ai;
    try {
      ai = getAIClient();
    } catch (e: any) {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            title: { type: Type.STRING, description: "The name/title of the novel." },
            subtitle: { type: Type.STRING, description: "A captivating subtitle or tagline." },
            author: { type: Type.STRING, description: "The author's name or pen name." },
            genre: { type: Type.STRING, description: "Must be strictly one of: 'cyberpunk', 'space-opera', or 'fantasy'." },
            logline: { type: Type.STRING, description: "A captivating 1-2 sentence story elevator pitch/logline." }
          },
    ...
    // Return gracefully to prevent crashing
    return res.json({
      success: true,
      title: "The Neon Spires",
      subtitle: "A Cyberpunk Odyssey of Sector 4",
...
// API route for generating a back-cover blurb / synopsis
...
// API route to continue/draft the next paragraph
...
// API route to rewrite selected text in a specific style
...
// API route to chat with the Lore Oracle / World Builder
app.post("/api/copilot/chat", async (req, res) => {
  try {
    ...
    try {
      ...
      // Offline fallback chat responder
    ...
    // Format chat history for Gemini contents array if provided, or build a simple text prompt
...
// API route to check if text is AI-generated (Humanize Checker)
app.post("/api/copilot/humanize-check", async (req, res) => {
  try {
    ...
    // Baseline offline heuristic check for typical AI words
    ...
    let score = 98; // Start with high human score
    ...
    try {
      ...
      // Return offline heuristic score
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            score: { type: Type.INTEGER, description: "Stylistic score from 0 to 100" },
            reasoning: { type: Type.STRING, description: "Concise analysis details" }
          },
...
// API route to convert text to organic human style (Humanize Converter)
app.post("/api/copilot/humanize-convert", async (req, res) => {
  try {
    ...
    try {
      ...
      // Offline fallback: do some basic cleanups of common AI words
      let cleanText = text
...
"${text}"`;
    ...
    return res.json({ success: true, text: humanized?.trim() || text });
  } catch (error: any) {
...
async function setupVite() {
  if (process.env.NODE_ENV !== "production") {
    ...
    // In a packaged Electron app process.cwd() is not the project root, so we
    // allow the host to point us at the real dist folder.
...
setupVite().then(() => {
  app.listen(PORT, "0.0.0.0", () => {
    // Keep this exact prefix — the Electron main process watches for it to know
    // the server is ready before opening the window.
